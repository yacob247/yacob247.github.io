const GAMES = [
  ...
  },
  {
    ...
    "steam": "",
    "epic": "https://www.epicgames.com/fortnite/en-US/home",
    "gog": ""
  },
  {
    "id": "apex",
    ...
    "steam": "https://store.steampowered.com/app/1172470/",
    "epic": "https://store.epicgames.com/en-US/p/apex-legends",
    "gog": ""
  },
  {
    "id": "cyberpunk2077",
  ...
  },
  {
    ...
    "steam": "https://store.steampowered.com/app/570/",
    "epic": "",
    "gog": ""
  },
  {
  ...
  },
  {
    ...
    "steam": "https://store.steampowered.com/app/294100/",
    "epic": "",
    "gog": ""
  },
  {
...
function loadAdminEntries() {
  try {
    const stored = localStorage.getItem('admin_entries');
    return stored ? JSON.parse(stored) : [];
  } catch(e) { return []; }
}

const ADMIN_ENTRIES = loadAdminEntries();
const ADMIN_GAMES = ADMIN_ENTRIES.filter(e => e.type !== 'tool');
const ADMIN_TOOLS = ADMIN_ENTRIES.filter(e => e.type === 'tool');

const ALL = [...GAMES, ...ADMIN_GAMES, ...TOOLS, ...ADMIN_TOOLS];
...
function gradeColor(g){
  return {
    ...
    'B+':'#3b82f6', 'B':'#8b5cf6', 'B-':'#d946ef'
  }[g]||'#71717a';
}

function assessmentFor(e){
  if (e.type === 'tool') {
    return [
      { label:'Use case', text:e.tagline || 'Useful when the workflow matches your project.' },
      { label:'Capabilities', text:(e.features || [])[0] || 'Focused utility with a simple access path.' },
      { label:'Access', text:e.price || 'Check the official site before relying on it for production work.' }
    ];
  }

  const genre = e.genre || 'game';
  return [
    { label:'Best for', text:`Players who want a ${genre.toLowerCase()} experience with ${((e.pros || [])[0] || e.tagline || 'a strong core loop').toLowerCase()}.` },
    { label:'Standout', text:(e.pros || [])[1] || (e.pros || [])[0] || e.tagline || 'A clear identity compared with similar releases.' },
    { label:'Watch for', text:(e.cons || [])[0] || 'No major caveat in the current review notes.' },
    { label:'Value read', text:e.price ? `${e.price} price point; judge it against your time, genre preference, and tolerance for its rough edges.` : 'Value depends on your genre preference and available play time.' }
  ];
}

function renderAssessment(e, compact=false){
  return `<div class="${compact ? 'assessment-preview' : 'assessment-grid'}">
    ${assessmentFor(e).slice(0, compact ? 3 : 4).map(item => `
...
let activeType='Games', activeGrade='All', activeGenre='All', searchVal='';
...
function getEntries(){
  return ALL.filter(e=>{
    ...
    const gradeOk = activeType==='Tools'||activeGrade==='All'||e.grade===activeGrade;
    const genreOk = activeType==='Tools'||activeGenre==='All'||e.genre===activeGenre;
...
function renderCards(){
  ...
  grid.innerHTML = entries.map(e=>{
    ...
    let badgeHtml = '';
    if(e.grade) {
      const textColor = ['#eab308','#22c55e','#10b981'].includes(gc) ? '#000' : '#fff';
      badgeHtml = `<span class="badge" style="background:${gc};color:${textColor}">${e.grade}</span>`;
    } else {
      badgeHtml = `<span class="badge" style="background:rgba(255,255,255,0.1);color:#fafafa">${e.category||'Tool'}</span>`;
    }
...
function openModal(id){
  ...
  // Only add buttons if the link actually exists in the object.
  let linksStr = '';
  if (e.steam) linksStr += `<a href="${e.steam}" target="_blank">🎮 Steam</a>`;
  if (e.epic) linksStr += `<a href="${e.epic}" target="_blank">⚡ Epic Games</a>`;
  if (e.gog) linksStr += `<a href="${e.gog}" target="_blank">🦉 GOG.com</a>`;
  if (e.mobile) linksStr += `<a href="${e.mobile}" target="_blank">🌐 Official Site</a>`;
  
  // Only show the "Access/Where to Buy" section if we have at least one link
  const linksSection = linksStr ? `
    <h2>${isTool ? 'Access' : 'Where to Buy'}</h2>
    <div class="modal-links">${linksStr}</div>
  ` : '';
  ...
  if(e.grade){
      const textColor = ['#eab308','#22c55e','#10b981'].includes(gc) ? '#000' : '#fff';
      badgeHtml = `<span class="badge" style="background:${gc};color:${textColor};font-size:1rem;padding:0.4rem 1rem;vertical-align:middle;margin-right:1rem;">${e.grade}</span>`;
  }
  ...
  // Update URL hash directly without snapping the page scroll
...
function closeModal(){
  ...
  // Remove hash from URL perfectly so you can share the main page link
...
function syncFilterVisibility() {
  ...
  if (gameFilters) gameFilters.style.display = activeType === 'Tools' ? 'none' : '';
  if (toolFilters) toolFilters.style.display = activeType === 'Tools' ? '' : 'none';
}

document.querySelectorAll('[data-type], [data-tab]').forEach(btn=>{
  btn.addEventListener('click',()=>{
    document.querySelectorAll('[data-type], [data-tab]').forEach(b=>b.classList.remove('active', 'type-active'));
    btn.classList.add('active');
    activeType = btn.dataset.type || (btn.dataset.tab === 'tools' ? 'Tools' : 'Games');
...
GENRES.forEach(g=>{
  let btn = Array.from(genreRow.querySelectorAll('[data-genre]')).find((button) => button.dataset.genre === g);
  if (!btn) {
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Check if someone pasted a direct link with a Hash (#rdr2)
  if(window.location.hash) {
    const targetId = window.location.hash.slice(1);
    // Slight delay ensures the DOM is fully prepped before modal animation
    setTimeout(() => { openModal(targetId); }, 50);
  }
});
}
