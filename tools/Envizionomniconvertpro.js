        ...
        function renderToolUI(id) {
            ...
            switch (id) {
                ...
                case "image-translator":
                    html = `<div class="grid grid-cols-1 lg:grid-cols-2 gap-8"><div class="space-y-4"><div class="border-2 border-dashed border-slate-700 hover:border-brand-500 rounded-2xl p-8 text-center cursor-pointer bg-slate-900/50" onclick="document.getElementById('trans-img-input').click()"><input type="file" id="trans-img-input" accept="image/*" class="hidden" onchange="translateImage(event)"><i class="fa-solid fa-language text-5xl text-slate-500 mb-3 block"></i><span class="text-sm font-medium text-slate-300 block">Select Image to Translate</span></div><label for="translation-lang" class="block text-xs font-semibold text-slate-400">Translate detected English text to</label><select id="translation-lang" aria-label="Translation target language" class="w-full bg-slate-900 border border-slate-700 p-3 rounded-xl text-slate-200"><option value="es">Spanish</option><option value="fr">French</option><option value="de">German</option><option value="it">Italian</option><option value="pt">Portuguese</option><option value="nl">Dutch</option><option value="pl">Polish</option><option value="tr">Turkish</option><option value="ru">Russian</option><option value="zh-CN">Chinese (Simplified)</option><option value="ja">Japanese</option><option value="ko">Korean</option><option value="ar">Arabic</option><option value="hi">Hindi</option></select><p class="text-xs text-slate-500">OCR currently reads English source text, then translates it using the selected target language.</p></div><div class="space-y-4"><textarea id="translation-out" rows="10" class="w-full p-4 bg-slate-900 border border-slate-700 rounded-xl text-sm font-mono text-slate-200" placeholder="Awaiting image upload..."></textarea></div></div>`;
                    break;
                case "qr-scanner":
